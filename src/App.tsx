...
function AppInner() {
  ...
  // Auth & Subscription Context for in-app trial monitoring
  const {
    currentUser,
    openSubscriptionModal
  } = useAuthSubscription();

  // Free Trial Remaining Time Monitor (168-hour free trial)
  const [trialCurrentTime, setTrialCurrentTime] = useState<number>(Date.now());
  const [dismissTrialWarningBanner, setDismissTrialWarningBanner] = useState<boolean>(false);

  useEffect(() => {
    const timer = setInterval(() => {
      setTrialCurrentTime(Date.now());
    }, 1000);
    return () => clearInterval(timer);
  }, []);

  // Compute remaining hours and expiration status
  const isUserTrialing = currentUser?.subscription?.status === 'trialing';
  const trialStartTimestamp = currentUser?.subscription?.trialStartDate
    ? new Date(currentUser.subscription.trialStartDate).getTime()
    : (currentUser?.createdAt ? new Date(currentUser.createdAt).getTime() : trialCurrentTime);

  const trialEndTimestamp = currentUser?.subscription?.trialEndDate
    ? new Date(currentUser.subscription.trialEndDate).getTime()
    : trialStartTimestamp + 7 * 24 * 60 * 60 * 1000;

  const trialMsLeft = Math.max(0, trialEndTimestamp - trialCurrentTime);
  const trialHoursLeft = Math.floor(trialMsLeft / (1000 * 60 * 60));
  const trialMinutesLeft = Math.floor((trialMsLeft % (1000 * 60 * 60)) / (1000 * 60));
  const trialSecondsLeft = Math.floor((trialMsLeft % (1000 * 60)) / 1000);

  // Condition: In-app warning banner triggers if less than 48 hours remain
  const showTrialExpiringWarningBanner =
    isUserTrialing &&
    trialHoursLeft < 48 &&
    trialMsLeft > 0 &&
    !dismissTrialWarningBanner;

  const isTrialFullyExpired =
    currentUser?.subscription?.status === 'expired_trial' ||
    (isUserTrialing && trialMsLeft <= 0);
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col transition-all duration-500 ease-in-out ${
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 transition-all duration-500 ease-in-out">
        ...
        {/* IN-APP FREE TRIAL EXPIRATION WARNING BANNER (< 48 Hours Remaining) */}
        {showTrialExpiringWarningBanner && (
          <div
            id="app-trial-expiring-warning-banner"
            className="mb-6 p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-amber-500/15 via-orange-500/10 to-amber-500/15 dark:from-amber-950/70 dark:via-orange-950/50 dark:to-amber-950/70 border-2 border-amber-400 dark:border-amber-700/80 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2"
          >
            <div className="flex items-start gap-3.5">
              <div className="p-2.5 rounded-xl bg-amber-500 text-white shrink-0 shadow-xs animate-bounce">
                <AlertTriangle className="w-5 h-5" />
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="text-sm sm:text-base font-black text-amber-950 dark:text-amber-100 flex items-center gap-2">
                    <span>⚠️ Free Trial Expiring Soon: Only {trialHoursLeft}h {trialMinutesLeft}m Remaining!</span>
                  </h3>
                  <span className="px-2 py-0.5 rounded-full bg-amber-600 text-white font-extrabold text-[10px] tracking-wide uppercase">
                    &lt; 48 Hours Left
                  </span>
                </div>
                <p className="text-xs text-amber-900/90 dark:text-amber-200/90 mt-1 max-w-3xl leading-relaxed">
                  Your 7-day free trial (168 hours) is coming to an end. After {trialHoursLeft}h {trialMinutesLeft}m {trialSecondsLeft}s, unthrottled AI coaching scorecards, voice pitch recording, and MEDDIC pipelines will pause. Select a plan now to retain full uninterrupted platform intelligence.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 shrink-0 self-end md:self-center w-full md:w-auto justify-end">
              <button
                ...
                className="px-3.5 py-2 rounded-xl text-xs font-bold bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 hover:bg-amber-50 dark:hover:bg-slate-700 border border-amber-300 dark:border-amber-700/60 shadow-xs transition-all cursor-pointer"
              >
                View Plans
              </button>

              <button
                id="banner-upgrade-subscription-btn"
                onClick={() => setActiveTab('billing')}
                className="px-4 py-2 rounded-xl bg-[#800000] hover:bg-[#600000] text-white font-black text-xs shadow-md flex items-center gap-2 transition-all cursor-pointer"
              >
                <CreditCard className="w-3.5 h-3.5 text-[#A8C66C]" />
              ...
              <button
                ...
                onClick={() => setDismissTrialWarningBanner(true)}
                className="p-2 rounded-xl text-amber-800 dark:text-amber-400 hover:bg-amber-200/50 dark:hover:bg-amber-900/40 transition-all cursor-pointer"
        ...
        {/* IN-APP FREE TRIAL EXPIRED BANNER */}
        {isTrialFullyExpired && (
          <div
            id="app-trial-expired-banner"
            className="mb-6 p-4 sm:p-5 rounded-2xl bg-red-600 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-2 border-red-700 animate-in fade-in"
          >
            <div className="flex items-start gap-3.5">
              <div className="p-2.5 rounded-xl bg-black/30 text-amber-300 shrink-0">
                <Lock className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-sm sm:text-base font-black flex items-center gap-2">
                  <span>🔒 7-Day Free Trial Expired (168 Hours Completed)</span>
                </h3>
                <p className="text-xs text-red-100 mt-1 max-w-3xl leading-relaxed">
                  Your complimentary 7-day access has ended. Upgrade to the Monthly Pro Plan ($15.99 CAD/mo) or Annual Pro Plan ($155.99 CAD/yr — Save 18%) to instantly restore access.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 shrink-0 self-end md:self-center w-full md:w-auto justify-end">
              <button
                onClick={() => setActiveTab('billing')}
                className="px-4 py-2 rounded-xl bg-white hover:bg-slate-100 text-red-700 font-black text-xs shadow-md flex items-center gap-2 transition-all cursor-pointer"
              >
                <CreditCard className="w-4 h-4 text-[#800000]" />
                <span>Restore Access in Billing</span>
                <ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
